...
import com.vaadin.examplefeature.ai.component.AiFileReceiver;
import com.vaadin.examplefeature.ai.component.AiInput;
import com.vaadin.examplefeature.ai.component.AiMessage;
import com.vaadin.examplefeature.ai.component.AiMessageList;
import com.vaadin.examplefeature.ai.component.InputSubmitListener;
 ...
 * </p>
 * <p>
 * <b>Note:</b> This class is not serializable. If your application uses
...
public class AiOrchestrator {
    ...
    private void configureFileReceiver() {
        ...
        fileReceiver.addFileUploadListener(event -> {
            var isDuplicate = pendingAttachments.stream()
                    .anyMatch(a -> a.fileName().equals(event.fileName()));
            if (isDuplicate) {
                LOGGER.warn("Duplicate file name ignored: {}", event.fileName());
                return;
            }
            ...
            LOGGER.debug("Added attachment: {}", event.fileName());
        });
    ...
    /**
     * Wrapper for Flow Upload component using MultiFileMemoryBuffer.
     */
    private static class UploadWrapper implements AiFileReceiver {
        private final Upload upload;
        private final MultiFileMemoryBuffer buffer;
        private Consumer<FileUploadEvent> uploadListener;
        private Consumer<String> removedListener;
        ...
        UploadWrapper(Upload upload) {
            ...
            upload.addSucceededListener(event -> {
                if (uploadListener != null) {
                    try {
                        ...
                        uploadListener.accept(new FileUploadEvent() {
                            @Override
                            public String fileName() {
                                return fileName;
                            }
            ...
            upload.addFileRemovedListener(event -> {
                if (removedListener != null) {
                    removedListener.accept(event.getFileName());
                }
            });
        }

        @Override
        public void addFileUploadListener(Consumer<FileUploadEvent> listener) {
